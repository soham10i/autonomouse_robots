"""Single source of truth for the ROSbot controller.

This is the Maze4 navigation stack (the same proven code as Maze3's
mak_03_controller — identical local_planner + config) ported wholesale into the
Maze1 ``mak_02_controller``.  The robot localises in a START-RELATIVE frame
(pose begins at the origin, IMU-seeded heading) and DISCOVERS both pillars by
vision, so the Maze4 world-coordinate facts below are documentation only — the
navigation does not depend on them.  Only the grid extent, exploration radius,
mapping resolution, and the floating-wall height band were retuned for Maze1
(see GRID_*, EXPL_MAX_RADIUS_M, AUX_Z_MIN).  Maze1 has green poison + tumbled
floating walls (WallShort(5) z 0.175-0.225, WallMedium(3) bottom 0.20); the
depth aux layer + costmap handle them exactly as the low panels in Maze4.
...
Maze4 facts that drove the original tuning (read directly from
``Maze4/worlds/Maze4.wbt``, verified with exact axis-angle rotation maths):
...
# ===========================================================================
# Maze1 integration: finer than Maze4's 0.04 for higher mapping quality (the
# previous Maze1 controller mapped at 0.025), and the arena is smaller than
# Maze4's so a 9 m grid covers it with margin (start-relative pose at origin).
GRID_RESOLUTION = 0.025         # m / cell (finer map: sharper walls, accurate
                                # floating-wall footprints; HARD_OBS_DIST etc. are
                                # converted via round(dist/res) so they still match
                                # the 0.116 m footprint at this resolution)
GRID_SIZE_M = 9.0               # square side; covers Maze1's arena from start origin
GRID_CELLS = int(round(GRID_SIZE_M / GRID_RESOLUTION))   # 360
GRID_ORIGIN = (-4.5, -4.5)      # world coord of cell (0,0) lower-left corner
...
AUX_Z_MIN = 0.04                # m above floor.  Maze1 integration: raised from
                                # Maze4's 0.01 because Maze1's floating walls sit
                                # HIGH (WallShort(5) z 0.175-0.225, WallMedium(3)
                                # bottom 0.20) — 0.04 avoids floor-noise false marks
                                # while still catching them in the [0.04,0.22] band.
...
AUX_LIDAR_BLIND_CELLS = 6        # skip a depth hit within this many cells of a lidar wall
                                 # (Maze4 used 4 @ 0.04 m = 0.16 m; 6 @ 0.025 m keeps
                                 # the same ~0.15 m physical reconcile distance)
...
EXPL_MAX_RADIUS_M = 4.0         # m from start; do not chase frontiers past this.
                                # Maze1 integration: both pillars are within ~2.5 m
                                # of start, so 4 m keeps a margin while preventing
                                # the robot wandering onto the open floor outside.
