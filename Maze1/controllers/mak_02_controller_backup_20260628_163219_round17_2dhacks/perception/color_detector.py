...
class ColorDetector:
    ...
    def _detect_one(self, hsv, depth, lo, hi, min_pixels):
        ...
        # A blob clipped by the LEFT/RIGHT image border is a PARTIAL view — the
        # pillar runs out of frame, so it is being seen obliquely / through a gap
        # and its centroid+depth are biased (the depth at the centroid can read an
        # occluding edge, not the pillar).  Flag it so the mission refuses to
        # CONFIRM the pillar from such a view (it may still update the estimate).
        m = S.PILLAR_EDGE_MARGIN_PX
        side_clipped = bool(u_left <= m or u_right >= self.w - 1 - m)
        return {"u": cu, "v": cv, "area": area, "bearing": bearing,
                "range": rng, "est_height": est_h, "aspect": aspect,
                "pix_w": pix_w, "side_clipped": side_clipped}
