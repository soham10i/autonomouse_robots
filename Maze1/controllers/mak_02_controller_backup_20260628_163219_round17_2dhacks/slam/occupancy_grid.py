    ...
    def mark_free_disc(self, wx, wy, radius):
        ...
        for dx in range(-n, n + 1):
            for dy in range(-n, n + 1):
                ...
                # A cell the robot is standing in cannot hold a floating wall, so
                # wipe any displaced/unstable depth_obs that landed on it (the
                # tumbled WallShort(5) mapped ~0.15 m off seals the robot into a
                # phantom wall otherwise — the gap-wedge).  Depth is blind < 0.6 m
                # so it never re-marks the footprint; the true wall, which the
                # robot never drives through, keeps its mark.
