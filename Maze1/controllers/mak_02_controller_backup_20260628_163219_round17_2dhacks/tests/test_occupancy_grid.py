...
class TestOccupancyGrid(unittest.TestCase):
    ...
    def test_mark_free_disc_clears_depth_obs_under_the_robot(self):
        # A displaced floating-wall mark that lands where the robot is standing
        # must be wiped (it cannot be a real wall there) so it can't seal the
        # robot in a phantom wall (the gap-wedge).  Barrier/poison are preserved.
        g = OccupancyGrid()
        cx, cy = 1.0, 1.0
        ix, iy = g.world_to_grid(cx, cy)
        g.depth_L[ix, iy] = S.DEPTH_L_MAX
        self.assertTrue(g.depth_obs_mask()[ix, iy])
        g.mark_free_disc(cx, cy, 0.10)
        self.assertFalse(g.depth_obs_mask()[ix, iy], "footprint depth_obs not cleared")

    def test_segment_clear_blocks_through_wall_sighting(self):
        # Clear path -> True.  A wall on the segment -> False.  A wall only within
        # stop_short of the endpoint (the pillar's own cell) -> still True.
        g = OccupancyGrid()
        self.assertTrue(g.segment_clear(0.0, 0.0, 1.0, 0.0))
        ix, iy = g.world_to_grid(0.5, 0.0)              # wall midway
        g.L[ix, iy] = S.L_MAX
        self.assertFalse(g.segment_clear(0.0, 0.0, 1.0, 0.0))
        # a floating wall (depth_obs) blocks too
        g2 = OccupancyGrid()
        jx, jy = g2.world_to_grid(0.5, 0.0)
        g2.depth_L[jx, jy] = S.DEPTH_L_MAX
        self.assertFalse(g2.segment_clear(0.0, 0.0, 1.0, 0.0))
        # wall 0.1 m before the endpoint, ignored by stop_short=0.25
        g3 = OccupancyGrid()
        kx, ky = g3.world_to_grid(0.9, 0.0)
        g3.L[kx, ky] = S.L_MAX
        self.assertTrue(g3.segment_clear(0.0, 0.0, 1.0, 0.0, stop_short=0.25))
