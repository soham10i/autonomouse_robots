...
class PillarTracker:
    """Running-mean world position of each pillar from camera detections."""
...
class Mak02Controller:
    ...
    def _pillar_in_reach(self, name):
        """True if the sought pillar is known and the robot is within
        PILLAR_REACH_DIST of it — i.e. the mission target is achieved, regardless
        of whether we are in EXPLORE or GO."""
    ...
    def _do_go(self, name, ret_state):
        ...
        if pose_distance(self.corrected, target) < S.PILLAR_REACH_DIST:
    ...
    def _do_recovery(self):
        """Reverse straight out of the wedge, THEN rotate toward the widest open
        direction the lidar sees — the simple, deterministic escape the user
        asked for (no wobble: back up, turn to space, go)."""
        ...
        if self.recovery_phase == "reverse":
            ...
            # finished reversing -> commit a rotation toward the open side
