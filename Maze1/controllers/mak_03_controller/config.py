...
# ===========================================================================
GRID_RESOLUTION = 0.025         # m / cell.  Raised resolution (was 0.04, Maze2-proven
                                # at 0.025): finer cells give the genuine ~0.3 m gap to
                                # the BLUE pillar a multi-cell free band so marginal
                                # poison/aux inflation can't slam it shut, and they put
                                # a sharp footprint on green patches and the floating
                                # wall instead of a coarse 4 cm blob.
GRID_SIZE_M = 8.0               # Maze1 arena is ~4.2 x 4.7 m; the robot starts off-
                                # centre so the start-relative odom frame spans about
                                # x[-1.3,3.0] y[-2.8,1.9] -> an 8 m box (origin -4,-4)
                                # covers it with margin (was 11.0 for Maze3's larger arena)
GRID_CELLS = int(round(GRID_SIZE_M / GRID_RESOLUTION))   # 320 @ 0.025
GRID_ORIGIN = (-4.0, -4.0)      # world coord of cell (0,0) lower-left corner (Maze1)

L_OCC = 0.85                    # log-odds added on a beam endpoint
L_FREE = -0.40                  # log-odds added on a traversed cell
L_MIN = -2.0                    # teleop_mapping value (was -2.5)
L_MAX = 3.5
# CRISP-MAP (teleop_mapping parity): a cell is "occupied" only once its log-odds
# clears 0.85 (~two consistent beam endpoints), not 0.50.  The old 0.50 flagged a
# wall from a single noisy beam which -- combined with the scan-matcher pose
# jitter (now disabled below) -- thickened every wall across 2-3 cells (the "poor
# resolution" smear).  teleop_mapping uses 0.85 on a STABLE odom pose -> crisp map.
L_OCC_THRESH = 0.85             # L above this => occupied (was 0.50)
L_FREE_THRESH = -0.40           # L below this => free (teleop value, was -0.35)
...
# RANGE-CONFIDENCE protection ("keep the accurate close points"): a wall/pillar
# endpoint seen from within OCC_CONFIRM_RANGE -- where the 2-D lidar is highly
# accurate -- and accumulated to L_CONFIRM_LEVEL (several consistent beams, not
# one noisy return) is LATCHED into mapping.occ_confirmed.  Latched cells are
# immune to free-ray erosion and are released only where the robot physically
# drives (mark_free_disc).  Stops the thin-wall/pillar self-erosion that emptied
# the map; the level gate keeps a single stray beam from latching a phantom.
OCC_CONFIRM_RANGE = 2.5         # m, lidar trusted as ground-truth within this range
L_CONFIRM_LEVEL = 2.0          # log-odds (~3 consistent endpoints @ L_OCC=0.85) before latching
OCC_CONFIRM_RELEASE = 8.0       # free-ray passes THROUGH a latched cell before it is un-latched.
                                # Makes the latch SELF-CLEANING: a real wall is never crossed by a
                                # free ray (stays); a drift-smear phantom in open space is crossed
                                # every scan -> released in ~8 frames.  Without this the latch was
                                # permanent and pivot drift-smear accreted forever -> caged robot.
...
AUX_LIDAR_BLIND_CELLS = 6        # a depth hit is kept ONLY where the 2-D lidar is genuinely
                                 # blind -- no lidar-occupied cell within this many cells
                                 # (~0.15 m at GRID_RESOLUTION 0.025; was 4 @ 0.04).  Anti-smear
...
# AUX_CONFIRM_LEVEL: DEPRECATED/UNUSED.  An earlier build latched a saturated aux
# (floating-wall) cell into the permanent occ_confirmed layer; reverted because the
# lidar sees straight under a floating wall so that latch never strike-released and
# dropped a permanent ~50-cell blob that caged the robot.  The aux confidence layer
# (with blind-shell-protected decay) already carries the floating wall.  Kept only so
# any stale reference does not crash; not read by mapping anymore.
AUX_CONFIRM_LEVEL = 12.0
...
# ===========================================================================
SM_ENABLED = True               # RE-ENABLED, conservative.  Disabling it (teleop
                                # parity) let the map erase its own walls on long,
                                # pivot-heavy runs: with no alignment, every-tick
                                # free-rays grazed past thin walls and cleared them,
                                # flooding the aux layer and oscillating the costmap
                                # (Maze1 looping bug).  Aligning each scan to the map
                                # before integration keeps walls crisp AND de-drifts.
                                # The earlier smear came from ACCEPTING large jumps;
                                # acceptance below is now confident-AND-small only.
SM_MIN_TRAVEL_M = 0.06          # only correct after this much motion (also the integ. gate)
SM_MIN_TURN_RAD = math.radians(6.0)
SM_MAX_BEAMS = 160
SM_LIN_HALF = 0.12              # m search window
SM_LIN_STEP = 0.03              # m
SM_ANG_HALF = math.radians(5.0) # rad search window
SM_ANG_STEP = math.radians(2.5) # rad
SM_MIN_HIT_FRAC = 0.65          # min score (beam-on-wall fraction) to accept any correction
SM_MARGINAL_MAX_CORR = 0.05     # m, MAX spatial shift accepted for ANY correction -- a
                                # confident match that wants a bigger jump is a mis-
                                # association and is rejected (keep odom).  This is the
                                # "de-drift without jitter" gate.
SM_TRUST_HIT_FRAC = 0.85        # (retained for ref; no longer an unconditional-accept path)
...
EXPL_MAX_RADIUS_M = 4.0         # m from start; do not chase frontiers past this.
                                # (Maze1: BLUE ~2.00 m, YELLOW ~1.13 m from start in
                                #  the start-relative odom frame; 4.0 covers the whole
                                #  ~4.7 m arena while keeping the robot from wandering.)
...
# inflation (pois -> 822 phantom cells) the flat-floor projection caused.
GREEN_DEPTH_MIN = 0.6           # m, depth camera is blind nearer than this -> drop
GREEN_FLOOR_Z_TOL = 0.05        # m, |reconstructed height| below this counts as floor
                                # (rejects green-tinted wall/pillar pixels, which sit
                                #  well above the floor and so must NOT become poison)
...
POISON_HIT_CAP = 8.0           # confidence ceiling
# Bounded permanence (the "pois -> 822, never decays" fix).  The old design froze
# any cell that reached POISON_HIT_CAP forever (it was excluded from decay), so a
# single phantom mark was permanent.  Now EVERY cell -- capped or not -- loses
# POISON_GLOBAL_DECAY each frame, so a patch the robot no longer sees fades over
# ~15-20 s while a patch still in view is re-topped faster than it bleeds and
# stays lethal.  Tiny relative to INC=2, so real, repeatedly-seen poison persists.
POISON_GLOBAL_DECAY = 0.02     # confidence drained from every nonzero cell per frame
