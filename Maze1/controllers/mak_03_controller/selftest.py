...
def main():
    ...
    dets = per.update_pillars(img, depth, (0.0, 0.0, 0.0))
    ...
    check("depth-validated green: floor pixels project to poison", gw.shape[0] > 0)
    ...
    check("depth-validated green: wall-height green is rejected", gw_w.shape[0] == 0)
    ...
    check("green projection needs depth (no flat-floor fallback)",
          per.green_floor_world(img, None, (0.0, 0.0, 0.0)).shape[0] == 0)
    ...
    probe = _StuckProbe()
    probe._reset_progress = M.Mak03Controller._reset_progress.__get__(probe)
    is_stuck = M.Mak03Controller._is_stuck.__get__(probe)
