...
caller (mapping.OccupancyGrid.integrate_depth_obstacles) uses the hits to
reinforce a PERSISTENT per-cell confidence layer and the clears only to DECAY it
-- the decay never reaches inside the < depth_min blind shell, so a floating wall
the robot has closed in on is NOT forgotten (see config.py's AUX_* comment).
