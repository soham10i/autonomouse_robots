...
class ScanMatcher:
    ...
    def match(self, predicted_pose, body_pts, field):
        ...
        Returns ``(pose, info)`` where ``info`` carries the normalised score, the
        correction magnitude, and whether the match was accepted.  Acceptance is
        a robust two-band gate (see :data:`settings.SM_MIN_SCORE_FRAC`):

        * **strong** (``norm >= SM_TRUST_SCORE_FRAC``) — accept any correction.
        * **marginal** (``SM_MIN_SCORE_FRAC <= norm < SM_TRUST_SCORE_FRAC``) —
          accept ONLY a small correction (``<= SM_MARGINAL_MAX_CORR_M``); a
          marginal score that also wants to teleport the pose is the lost/
          ambiguous failure mode, so we dead-reckon on odometry instead.
        * **lost** (``norm < SM_MIN_SCORE_FRAC``) — reject, dead-reckon.
        """
        info = {"accepted": False, "score": 0.0, "norm": 0.0,
                "correction": 0.0, "reason": ""}
        ...
        correction = math.hypot(best_pose[0] - predicted_pose[0],
                                best_pose[1] - predicted_pose[1])
        info["score"] = best_score
        info["norm"] = norm
        info["correction"] = correction

        if norm >= S.SM_TRUST_SCORE_FRAC:
            info["accepted"] = True
            return best_pose, info
        if norm >= S.SM_MIN_SCORE_FRAC and correction <= S.SM_MARGINAL_MAX_CORR_M:
            info["accepted"] = True
            return best_pose, info
        info["reason"] = ("marginal-large-correction"
                          if norm >= S.SM_MIN_SCORE_FRAC else "low-score")
        return predicted_pose, info
