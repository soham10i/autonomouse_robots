...
class Mak02Controller:
    def __init__(self):
        ...
        self.monitor = FaultMonitor(
            os.path.join(self.maps_dir, S.DIAG_OUT_SUBDIR), enabled=S.DIAG_ENABLED)
        self._diag = {}                 # per-tick fields stashed by each subsystem
        self._pose_win = []             # (t, x, y) recent window for progress check
    ...
    def _read_scan(self):
        ...
        if ranges is None:
            self.scan_body = np.empty((0, 2))
            return
        body, valid = self.lidar_model.ranges_to_body(ranges)
        self.scan_body = body
        self._diag["lidar_valid"] = int(np.count_nonzero(valid))
        self._diag["lidar_total"] = int(valid.size)
    ...
    def _slam_step(self):
        ...
        if self.field.n_occupied >= 10:
            ...
            self._diag["sm_reason"] = info.get("reason", "")
        else:
    ...
    def _integrate_depth(self):
        """Fold the depth camera into the boolean aux obstacle layer (Maze4 port).

        ``thin_scan`` collapses the depth image to one in-band hit / clear
        sight-line per bearing; ``integrate_aux`` stamps hits and raytrace-CLEARS
        clear bearings every tick (no sticky counters, no decay), reconciled
        against the dilated lidar so only genuinely lidar-blind floating/low walls
        survive.  That keeps the floating-wall map reliable, so A* (which unions
        ``depth_obs_mask()`` into its costmap) routes AROUND the wall from far —
        the working Maze4 behaviour, instead of the old log-odds layer that
        flickered and let the robot drive into WallShort(5)."""
        if self.depth_model is None:
            return
        depth = self.io.read_depth()
        if depth is None:
            self._diag["depth_ok"] = False
            return
        bearings, hit_ranges, hit_mask, clear_mask = self.depth_model.thin_scan(depth)
        self.grid.integrate_aux(self.corrected, bearings, hit_ranges, hit_mask,
                                clear_mask, self.depth_model.depth_min)
        # Live floating-wall hit world points (diagnostics + _floating_wall_ahead).
        hb = bearings[hit_mask]
        hr = hit_ranges[hit_mask]
        if hb.size:
            ang = self.corrected[2] + hb
            self._last_aux_world = np.stack(
                [self.corrected[0] + hr * np.cos(ang),
                 self.corrected[1] + hr * np.sin(ang)], axis=1)
        else:
            self._last_aux_world = np.empty((0, 2))
        self._last_aux_t = self.sim_time
        self._accumulate_cloud(depth)
        # --- diagnostics: is the depth cam seeing a floating wall ahead? ---
        if S.DIAG_ENABLED:
            d = np.asarray(depth, dtype=np.float32)
            finite = np.isfinite(d)
            self._diag["depth_ok"] = True
            self._diag["depth_valid_frac"] = float(finite.mean()) if d.size else 0.0
            self._diag["depth_inband_hits"] = int(self._last_aux_world.shape[0])
            try:
                self._diag["depth_fwd"] = self.depth_model.forward_obstacle(depth)
            except Exception:
                self._diag["depth_fwd"] = {}
    ...
    def _enter(self, state):
        if self.state != state:
            ...
            self.monitor.event("FSM", f"{self.state} -> {state}",
    ...
    def _on_pillar_reached(self, name):
        if name == "blue":
            if self.t_blue is None:
                ...
                self.monitor.event("MISSION", f"BLUE reached pos={self.pillars.pos['blue']}",
                                   self.sim_time, self.tick)
    ...
    def _navigate(self, goal):
        ...
        v, w, dinfo = self.dwa.compute((float(carrot_body[0]), float(carrot_body[1])),
                                       obstacles)
        # diagnostics: DWA outcome + nearest obstacle (clearance) + plan length
        self._diag["dwa_boxed"] = bool(dinfo.get("boxed", False))
        self._diag["clearance_m"] = self._nearest_obstacle_dist(obstacles)
        self._diag["plan_len"] = len(self.plan) if self.plan else 0
        return v, w, False
    ...
    def _local_obstacles_body(self):
        """Live lidar + forward IR + mapped aux/poison/barrier nearby, body frame.

        The DWA's floating-wall input is the MAPPED, raytrace-cleared aux layer
        (``depth_obs_mask()``), exactly as in the working Maze4 build — NOT a live
        depth thin-scan.  The mapped aux is reliable (cleared every tick + lidar-
        reconciled) so it avoids the noisy/displaced live-thin-scan double-counting
        that choked narrow passages.  A* already routes around the same aux cells
        (they're unioned into the costmap), so by the time the DWA sees one the
        robot is being steered past it, not into it.  Forward IR adds close-range
        hits the lidar is blind to (<0.2 m).
        """
        scan = self.scan_body
    ...
    def _stuck_check(self, ret_state):
        ...
        if (self.sim_time - self._anchor_time) > S.STUCK_TIMEOUT_S:
            ...
            fw = self._floating_wall_ahead()
            self.monitor.event(
                "RECOVERY", f"stuck in {self.state}; fw_ahead={fw['dist']:.2f}m "
                f"src={fw['src']} clear={self._diag.get('clearance_m')}",
                self.sim_time, self.tick)
    ...
    def _virtual_bumper(self, cmd_v):
        ...
        if (self.sim_time - atime) > S.BUMPER_STALL_TIME_S:
            ...
            self.monitor.event(
                "BUMPER", f"invisible obstacle stamped @ ({bx:+.2f},{by:+.2f}) — "
                "forward push with no progress (likely a blind-zone floating wall)",
                self.sim_time, self.tick)
    ...
    # ----------------------------------------------------- diagnostics
    def _nearest_obstacle_dist(self, obstacles_body):
        o = np.asarray(obstacles_body, dtype=np.float64).reshape(-1, 2)
        if o.shape[0] == 0:
            return float("inf")
        return float(np.min(np.hypot(o[:, 0], o[:, 1])))

    def _forward_cone_nearest(self, body):
        """Nearest point of ``body`` (N,2) inside the forward cone -> dist or inf."""
        b = np.asarray(body, dtype=np.float64).reshape(-1, 2)
        if b.shape[0] == 0:
            return float("inf")
        ahead = b[(b[:, 0] > 0.0)
                  & (np.abs(np.arctan2(b[:, 1], b[:, 0])) < S.DIAG_FWD_CONE_RAD)]
        if ahead.shape[0] == 0:
            return float("inf")
        return float(np.min(np.hypot(ahead[:, 0], ahead[:, 1])))

    def _floating_wall_ahead(self):
        """Nearest FLOATING-wall obstacle ahead, from BOTH the live depth thin-scan
        and the mapped depth_obs/cloud cells.  Returns dist + which source is
        nearer + whether it is mapped at all + whether it's in the depth blind
        zone — the data that pins down why a floating wall isn't being avoided."""
        live = float("inf")
        aw = self._last_aux_world
        if aw.shape[0] and (self.sim_time - self._last_aux_t) < 0.5:
            live = self._forward_cone_nearest(
                inverse_transform_points(aw, *self.corrected))
        mapped = float("inf")
        mask = self.grid.depth_obs_mask() | self.grid.cloud_obs
        if mask.any():
            ix, iy = np.where(mask)
            wx = self.grid.origin[0] + (ix + 0.5) * self.grid.res
            wy = self.grid.origin[1] + (iy + 0.5) * self.grid.res
            rx, ry, _ = self.corrected
            near = (wx - rx) ** 2 + (wy - ry) ** 2 < S.DIAG_MAPPED_FW_RADIUS ** 2
            if near.any():
                world = np.stack([wx[near], wy[near]], axis=1)
                mapped = self._forward_cone_nearest(
                    inverse_transform_points(world, *self.corrected))
        dist = min(live, mapped)
        if not math.isfinite(dist):
            return {"dist": float("inf"), "src": None, "mapped": False, "blind": False}
        src = "mapped" if mapped <= live else "live_depth"
        return {"dist": dist, "src": src, "mapped": math.isfinite(mapped),
                "blind": dist < S.DEPTH_OBS_MIN_RANGE}

    def _plan_under_floating(self):
        if not self.plan:
            return False
        mask = self.grid.depth_obs_mask() | self.grid.cloud_obs
        if not mask.any():
            return False
        for px, py in self.plan:
            ix, iy = self.grid.world_to_grid(px, py)
            for dx in (-1, 0, 1):
                for dy in (-1, 0, 1):
                    jx, jy = ix + dx, iy + dy
                    if self.grid.in_bounds(jx, jy) and mask[jx, jy]:
                        return True
        return False

    def _progress_m(self):
        """Net travel over the last DIAG_PROGRESS_WINDOW_S (the no-progress test)."""
        self._pose_win.append((self.sim_time, self.corrected[0], self.corrected[1]))
        while self._pose_win and self.sim_time - self._pose_win[0][0] > S.DIAG_PROGRESS_WINDOW_S:
            self._pose_win.pop(0)
        if len(self._pose_win) < 2:
            return float("inf")
        x0, y0 = self._pose_win[0][1], self._pose_win[0][2]
        return math.hypot(self.corrected[0] - x0, self.corrected[1] - y0)

    def _collect_and_monitor(self, v, w):
        if not S.DIAG_ENABLED:
            return
        d = self._diag
        goal = self.plan_goal if self.state in (GO_BLUE, GO_YELLOW) else self.expl_goal
        snap = {
            "t": self.sim_time, "tick": self.tick, "state": self.state,
            "recovery_phase": self.recovery_phase,
            "lidar_valid": d.get("lidar_valid"), "lidar_total": d.get("lidar_total"),
            "depth_ok": d.get("depth_ok", True),
            "depth_valid_frac": d.get("depth_valid_frac"),
            "depth_inband_hits": d.get("depth_inband_hits"),
            "depth_fwd": d.get("depth_fwd", {}),
            "ir": [None if r is None else round(r, 2) for r in self.io.read_ranges()],
            "sm_norm": self._last_sm_norm, "sm_ok": self._last_sm_ok,
            "sm_reason": d.get("sm_reason", ""),
            "pose": [round(c, 3) for c in self.corrected],
            "occ": int(self.grid.occupied_mask().sum()),
            "dobs": int(self.grid.depth_obs_mask().sum()),
            "cobs": int(self.grid.cloud_obs.sum()),
            "vox": len(self.cloud) if self.cloud else 0,
            "bar": int(self.grid.barrier.sum()),
            "fw": self._floating_wall_ahead(),
            "cmd_v": v, "cmd_w": w,
            "dwa_boxed": d.get("dwa_boxed", False),
            "clearance_m": d.get("clearance_m"),
            "plan_len": d.get("plan_len", len(self.plan) if self.plan else 0),
            "plan_under_floating": self._plan_under_floating(),
            "goal_str": (None if goal is None
                         else f"({goal[0]:+.2f},{goal[1]:+.2f})"),
            "progress_m": self._progress_m(),
            "stuck_t": self.sim_time - self._anchor_time,
            "recov_chain": self.recovery_chain,
            "blue_known": self.pillars.known("blue"),
            "blue_conf": self.pillars.is_confirmed("blue"),
            "yellow_known": self.pillars.known("yellow"),
            "yellow_conf": self.pillars.is_confirmed("yellow"),
        }
