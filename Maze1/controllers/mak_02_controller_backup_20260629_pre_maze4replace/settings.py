...
V_MIN = 0.04                    # m/s
W_MAX = 1.6                     # rad/s (lowered from 2.0: gentler turns, less
...
AUX_Z_MIN = 0.04                # m above floor (Maze1 floating walls sit high:
                                # WallShort(5) z 0.175-0.225, WallMedium(3) ≥0.20)
AUX_Z_MAX = ROBOT_HEIGHT        # m (== 0.22, high-float passages stay open)
AUX_MAX_RANGE = 1.2             # m, drop far/noisy depth pixels (Maze4 used 1.0;
                                # 1.2 maps the wall a touch earlier for A* to route)
AUX_MIN_HITS = 3                # a cell must be hit this many times before it
                                # becomes a confirmed aux obstacle (noise gate)
# --- Maze4-ported boolean aux raytrace-clearing (the working design) ---------
# integrate_aux() stamps a hit and CLEARS the sight-line every tick (no sticky
# counters, no decay).  These mirror Maze4's AUX_* knobs, scaled to Maze1's finer
# 0.025 m grid: AUX_LIDAR_BLIND_CELLS keeps the same ~0.15 m physical reconcile
# distance (Maze4 used 4 cells @ 0.04 m = 0.16 m → 6 cells @ 0.025 m).
AUX_CLEAR_MARGIN = 0.05         # m; stop raytrace-clearing this short of a hit
AUX_COL_SUBSAMPLE = 160         # cap depth-image columns rasterised per tick (speed)
AUX_LIDAR_BLIND_CELLS = 6       # skip/retire a depth hit within this many cells of a
                                # lidar wall (drops normal-wall duplicates; keeps
                                # genuinely lidar-blind floating/low panels)
...
# DWA hard-reject radius.  Maze4's lesson: keep this at the footprint AND matched
# to the A* hard clearance (ROBOT_RADIUS - HARD_OBS_MARGIN = 0.10) so the local
# planner threads exactly the gaps A* plans — no narrower (it refuses A*'s own
# path) and no wider (it grazes walls).  The old 0.08 sat BELOW the footprint, so
# a mapped floating cell 0.10-0.14 m ahead never tripped the rejection and the
# robot drove into WallShort(5); 0.10 makes the DWA reject at the footprint.
LP_SAFE_RADIUS = 0.10           # m; footprint half-width, matched to A* clearance
...
PILLAR_MIN_PIXELS = 15          # min coloured blob area (pixel count)
...
# ===========================================================================
# Fault detection & real-time monitoring (diagnostics.FaultMonitor)
# ===========================================================================
# Instruments every stage of the pipeline (sensors -> SLAM -> mapping ->
# perception -> planning -> DWA -> recovery -> FSM), writes a per-tick JSONL
# telemetry stream + an alerts log, and prints a live multi-section dashboard so
# you can SEE which subsystem is active/failing in real time — built to localise
# the floating-wall-not-avoided / corner-collision faults.
DIAG_ENABLED = True
DIAG_OUT_SUBDIR = "diag"             # maps/diag/{telemetry.jsonl, alerts.log}
DIAG_DASHBOARD_PERIOD_S = 1.0        # s; print the live dashboard this often
DIAG_TELEMETRY_EVERY_TICKS = 2       # write a JSONL telemetry row every N ticks
DIAG_FWD_CONE_RAD = math.radians(22.0)   # forward cone for "obstacle ahead" tests
DIAG_FW_WARN_DIST = 0.55             # m; warn when a floating wall is this close ahead
DIAG_FW_CRIT_DIST = 0.32             # m; critical (about to drive under/into it)
DIAG_SLAM_LOST_STREAK = 3            # consecutive rejected matches -> SLAM-lost alert
DIAG_BOXED_STREAK = 3                # consecutive DWA-boxed ticks -> boxed alert
DIAG_PROGRESS_WINDOW_S = 0.8         # s; window for the no-progress (collision) check
DIAG_PROGRESS_MIN_M = 0.02           # m; less travel than this while pushing = blocked
DIAG_CLEAR_TIGHT_M = 0.10            # m; clearance below this in recovery = can't turn
DIAG_MAPPED_FW_RADIUS = 1.0          # m; window for mapped floating-wall cells ahead
