...
class OccupancyGrid:
    def __init__(self):
        ...
        # Depth obstacle layer (floating walls the lidar is blind to).  PORTED
        # FROM Maze4 (the working design): a BOOLEAN layer that is raytrace-CLEARED
        # every tick — a hit SETS a cell, a clear depth sight-line CLEARS the ray,
        # with NO sticky-hit counters, NO decay heuristics, and reconciled against
        # a dilated lidar-occupied mask so only genuinely lidar-blind low/floating
        # panels survive.  This replaces the log-odds ``depth_L`` whose marks
        # flickered on/off (the FaultMonitor's constant DEPTH_UNMAPPED) and let the
        # robot drive into WallShort(5).  ``depth_obs_mask()`` now returns this.
        self.aux = np.zeros(shape, dtype=bool)
        # (legacy log-odds depth layer — kept only so old unit tests still import;
        # no longer in the planning pipeline.)
    ...
    # ----------------------------------------------- depth-aux (Maze4 port)
    def _dilate_mask(self, mask, n):
        """Return ``mask`` dilated by ``n`` cells (8-connected disc)."""
    ...
    def _ray_cells(self, pose, bearing_body, r0, r1):
        ...
        ixs = np.floor((fx - self.origin[0]) / self.res).astype(np.int32)
        iys = np.floor((fy - self.origin[1]) / self.res).astype(np.int32)
        ok = (ixs >= 0) & (ixs < self.cells) & (iys >= 0) & (iys < self.cells)
        return (ixs[ok].astype(np.int64) * self.cells + iys[ok].astype(np.int64))

    def integrate_aux(self, pose, bearings, hit_ranges, hit_mask, clear_mask,
                      depth_min):
        """Stamp/raytrace-clear the boolean depth-aux layer from one thin scan.

        PORTED FROM Maze4 (the working floating-wall design).  Per column:
          * a HIT sets the endpoint cell occupied and clears the ray from
            ``depth_min`` to ``range - AUX_CLEAR_MARGIN``;
          * a CLEAR sight-line clears the ray from ``depth_min`` to
            ``AUX_MAX_RANGE``.
        Clears are applied BEFORE sets so a hit's own partial-clear ray cannot
        erase its own (farther-out) endpoint.  Because clearing starts at
        ``depth_min`` (the camera's 0.6 m minimum range in the real run), a mark
        that has entered the blind zone is never erased — the equivalent of the
        old ``DEPTH_OBS_MIN_RANGE`` guard, achieved for free.
        ...
        lidar already maps; stamping those would duplicate the whole wall map into
        a lethal layer and box the robot in.  So a depth hit becomes an aux
        obstacle only where no lidar wall sits within ``AUX_LIDAR_BLIND_CELLS``.
        """
        n_total = bearings.shape[0]
        if n_total == 0:
            return
        sub = max(1, int(S.AUX_COL_SUBSAMPLE))
        if n_total > sub:
            idx = np.linspace(0, n_total - 1, sub).astype(int)
        else:
            idx = np.arange(n_total)

        blind = self._dilate_mask(self.occupied_mask(), int(S.AUX_LIDAR_BLIND_CELLS))
        ...
        for i in idx:
            if hit_mask[i]:
                ...
                clear_to = max(depth_min, rng - S.AUX_CLEAR_MARGIN)
                ...
                bearing = float(bearings[i])
                clear_lin.append(self._ray_cells(pose, bearing, depth_min,
                                                 S.AUX_MAX_RANGE))
        ...
        # wall (clears duplicates stamped before the lidar finished mapping it —
        # e.g. during the INIT_SCAN spin).  Genuine lidar-blind low panels are
        # never in ``blind``, so they survive.
        self.aux &= ~blind
        self._dirty = True

    def aux_points_near(self, wx, wy, radius):
        """World ``(M, 2)`` of aux-occupied cell centres within ``radius``."""
    ...
    def _mask_points_near(self, mask, wx, wy, radius):
        ...
        x0, x1 = max(0, cix - r), min(self.cells, cix + r + 1)
        y0, y1 = max(0, ciy - r), min(self.cells, ciy + r + 1)
        if x0 >= x1 or y0 >= y1:
            return np.empty((0, 2))
        lx, ly = np.nonzero(mask[x0:x1, y0:y1])
        if lx.size == 0:
            return np.empty((0, 2))
        wx_ = self.origin[0] + (x0 + lx + 0.5) * self.res
        wy_ = self.origin[1] + (y0 + ly + 0.5) * self.res
    ...
    def depth_obs_mask(self):
        """The floating-wall obstacle mask = the Maze4-ported boolean aux layer."""
        return self.aux
    ...
    def mark_free_disc(self, wx, wy, radius):
        ...
        for dx in range(-n, n + 1):
            for dy in range(-n, n + 1):
                ...
                # floating wall — wipe any (stale/displaced) aux or cloud mark
                # so a mis-projected slab can never box the robot in on its pose.
                self.aux[ix, iy] = False
