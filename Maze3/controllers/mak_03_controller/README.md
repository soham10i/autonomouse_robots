...
`Maze1/controllers/mak_03_controller/`, because Maze4 has 4 low wall panels the
...
The Rosbot node's `controller` field is already set to `mak_03_controller` in
...
early. Live map and snapshots are written to `maps/`.

## Files

| file | role |
|---|---|
| `mak_03_controller.py` | FSM, main loop, sensing/planning/driving orchestration |
