"""mak_02_controller — Maze5 frontier-exploration navigation for the ROSbot.
...
class Mak02Controller:
    ...
    #  Driving
    # ===================================================================== #
    def _poison_body(self):
        pw = self.grid.poison_points_near(self.pose[0], self.pose[1], C.DWA_SLOWDOWN_DIST + 0.4)
        if pw.shape[0] == 0:
            return np.empty((0, 2))
    ...
    def _drive_to(self, path, v_cap=None):
        ...
        v, w = self.dwa.compute(self.pose, carrot, self.scan_body, poison_b,
    ...
    def _is_stuck(self, t, commanding_forward):
    ...
    def _finalize(self):
        ...
        try:
            ...
            np.savez_compressed(os.path.join(self.outdir, "map.npz"),
                                L=self.grid.L, poison=self.grid.poison)
    ...
    def _log(self, t):
        ...
        print(f"[t={t:6.1f}] {self.state:13s} "
              ...
              f"v={self.cur_v:.2f} occ={occ} pois={pois} "
